...
const add = async (req, res) => {
    try {
        const { agenda, attendes, attendesLead, location, related, dateTime, createBy, timestamp } = req.body;

        const meetingData = { agenda, attendes, attendesLead, location, related, dateTime, createBy, timestamp };

        const meeting = new MeetingHistory(meetingData);

        const validationErrors = meeting.validateSync();

        if (validationErrors) res.status(400).json({ error: validationErrors.message });
...
const deleteMany = async (req, res) => {
    try {
        ...
        else return res.status(404).json({ success: false, message: "Failed to remove meetings" });
    } catch (err) {
